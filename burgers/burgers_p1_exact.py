...
class EQD:
    ...
    def loss(self, params, co, w, tau, v):
        ...
        library = jnp.concatenate((u * u_dx1, u_ddx1, u_dx1, u_dddx1, u_ddddx1, u, u**2, u**3, u**4, u**2 * u_dx1, u**3 * u_dx1, u**4 * u_dx1, u * u_ddx1, u**2 * u_ddx1, u**3 * u_ddx1, u**4 * u_ddx1,
    ...
    def spike_and_slab_EP(self, params):
        ...
        self.Kernel = Kernel(self.jitter, RBF_kernel_u_1d())
        self.K_list = jnp.array([self.Kernel.get_kernel_matrix(self.X_col[i], ls[i]) for i in range(self.X_col.shape[0])])
        self.cho_list = np.linalg.cholesky(self.K_list)
        self.K_inv_list = jnp.linalg.inv(self.K_list)
        mu = params['mu'].sum(axis=-1)
        mu = tl.tenalg.multi_mode_dot(mu, self.cho_list)
        self.cov_f = [self.Kernel.get_cov(self.sensors[i], self.X_col[i], ls[i]) for i in range(self.X_col.shape[0])]
    ...
    def train(self):
        params_f = {
            ...
            'log_ls': np.array([-0.5, -0.5]),
        }
        optimizer1 = optax.adam(5e-5)
        ...
        for i in range(800000):
            params_f, opt_state, _ = self.step(optimizer, params_f, opt_state, co, w, 10000, 50)
            if i + 1 == start_EQD:
                self.init_EP(params_f)
                w = self.spike_and_slab_EP(params_f)
                co = 1.0
                self.tau = 100
                optimizer2 = optax.adam(2e-6)
...
Xtrain = np.load('Xtrain_burgersp1.npy')
Ttrain = np.load('Ttrain_burgersp1.npy')
Xtest = np.load('Xtest_burgersp1.npy')
Ttest = np.load('Ttest_burgersp1.npy')
X_train = np.load('xtrain_burgersp1.npy')
Y_train = np.load('ytrain_burgersp1.npy')
X_test = np.load('xtest_burgersp1.npy')
Y_test = np.load('ytest_burgersp1.npy')
