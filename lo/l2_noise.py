...
class EQD(object):
    ...
    def init(self):
        ...
        self.s03 = 1.0
        self.INF3 = 1000000.0
        d3 = self.num_operators
        self.rho_p_full3 = np.zeros(d3)
        self.mu_p_full3 = np.zeros(d3)
        self.v_p_full3 = self.INF3 * np.ones(d3)
        ...
        self.s04 = 1.0
        self.INF4 = 1000000.0
        d4 = self.num_operators
        self.rho_p_full4 = np.zeros(d4)
        self.mu_p_full4 = np.zeros(d4)
        self.v_p_full4 = self.INF4 * np.ones(d4)
        ...
        self.s05 = 1.0
        self.INF5 = 1000000.0
        d5 = self.num_operators
        self.rho_p_full5 = np.zeros(d5)
        self.mu_p_full5 = np.zeros(d5)
        self.v_p_full5 = self.INF5 * np.ones(d5)
        ...
        self.s06 = 1.0
        self.INF6 = 1000000.0
        d6 = self.num_operators
        self.rho_p_full6 = np.zeros(d6)
        self.mu_p_full6 = np.zeros(d6)
        self.v_p_full6 = self.INF6 * np.ones(d6)
    ...
    def get_lib(self, params):
        ...
        self.deri_cov_times_K_inv_list3 = [[jnp.matmul(self.derivative_cov_list3[i][j], self.K_inv_list3[j, :]) for j in range(len(self.derivative_cov_list3[0]))]
                                           for i in range(len(self.derivative_cov_list3))]
        ...
        self.deri_cov_times_K_inv_list4 = [[jnp.matmul(self.derivative_cov_list4[i][j], self.K_inv_list4[j, :]) for j in range(len(self.derivative_cov_list4[0]))]
                                           for i in range(len(self.derivative_cov_list4))]
        ...
        self.deri_cov_times_K_inv_list5 = [[jnp.matmul(self.derivative_cov_list5[i][j], self.K_inv_list5[j, :]) for j in range(len(self.derivative_cov_list5[0]))]
                                           for i in range(len(self.derivative_cov_list5))]
        ...
        self.deri_cov_times_K_inv_list6 = [[jnp.matmul(self.derivative_cov_list6[i][j], self.K_inv_list6[j, :]) for j in range(len(self.derivative_cov_list6[0]))]
                                           for i in range(len(self.derivative_cov_list6))]
    ...
    def loss(self, params, co, w1, w2, w3, w4, w5, w6, tau, v):
        ...
        elbo = -KL - tau * jnp.sum(jnp.square(f_sample.reshape(-1) - self.Y1.reshape(-1))) - tau * jnp.sum(jnp.square(f_sample2.reshape(-1) - self.Y2.reshape(-1))) - tau * jnp.sum(
            jnp.square(f_sample3.reshape(-1) - self.Y3.reshape(-1))) - tau * jnp.sum(jnp.square(f_sample4.reshape(-1) - self.Y4.reshape(-1))) - tau * jnp.sum(
                jnp.square(f_sample5.reshape(-1) - self.Y5.reshape(-1))) - tau * jnp.sum(jnp.square(f_sample6.reshape(-1) - self.Y6.reshape(-1))) - co * v * jnp.sum(
                    jnp.square(u_dx1.reshape(-1, 1)[a:b] - jnp.matmul(library1.T, w1.reshape(-1, 1))[a:b])) - co * v * jnp.sum(
                        jnp.square(u_dx1_2.reshape(-1, 1)[a:b] - jnp.matmul(library2.T, w2.reshape(-1, 1))[a:b])) - co * v * jnp.sum(
                            jnp.square(u_dx1_3.reshape(-1, 1)[a:b] - jnp.matmul(library3.T, w3.reshape(-1, 1))[a:b])) - co * v * jnp.sum(
                                jnp.square(u_dx1_4.reshape(-1, 1)[a:b] - jnp.matmul(library4.T, w4.reshape(-1, 1))[a:b])) - co * v * jnp.sum(
                                    jnp.square(u_dx1_5.reshape(-1, 1)[a:b] - jnp.matmul(library5.T, w5.reshape(-1, 1))[a:b])) - co * v * jnp.sum(
                                        jnp.square(u_dx1_6.reshape(-1, 1)[a:b] - jnp.matmul(library6.T, w6.reshape(-1, 1))[a:b]))
        return -elbo.sum()
    ...
    def spike_and_slab_truncate(self, params):
        ...
        for it in range(1000):
            ...
            self.mu_p2 = self.v_p2 * v_p_inv_mu2
            self.r2 = self.rho_p2 + self.rho2
            # print(self.v2.shape, mu_tilt2.shape)
            self.S2 = np.linalg.inv(np.diag(1.0 / self.v_p2) + self.tau2 * (X2.T @ X2))
            self.mu2 = self.S2 @ (self.mu_p2 / self.v_p2 + self.tau2 * (X2.T @ y2))
            self.v2 = np.diag(self.S2)
            diff2 = np.sum((old_mu2 - self.mu2)**2)
...
X_train = np.load('xtrain_lo_0.1.npy')
Y_train = np.load('ytrain_lo_0.1.npy')
